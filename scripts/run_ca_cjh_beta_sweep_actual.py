#!/usr/bin/env python
# [AGENT-ADD] Beta sweep for CA-CJH actual-data experiments.
"""Run CA-CJH lpt_cjh beta sweep on actual panel-line data."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CA-CJH beta sweep on actual data")
    parser.add_argument("--config", default="config_self_label_diff.yaml")
    parser.add_argument("--output-dir", default="")
    parser.add_argument("--betas", default="0.0,0.3,0.5,0.7,0.9,1.0")
    ...
    output_dir = args.output_dir or os.path.join("PPO", "eval", "ca_cjh_sweeps", datetime.now().strftime("%Y%m%d_%H%M%S_beta_actual"))
    os.makedirs(output_dir, exist_ok=True)
    beta_values = [float(part.strip()) for part in str(args.betas).split(",") if part.strip()]

    rows: List[Dict[str, object]] = []
    for beta in beta_values:
        run_dir = os.path.join(output_dir, f"beta_{str(beta).replace('.', 'p')}")
        os.makedirs(run_dir, exist_ok=True)
        ca_cfg = dict((config.get("ca_cjh") or {}) if isinstance(config, dict) else {})
        ca_cfg.update({
            "feature_mode": "process_plus_core",
            "priority_mode": "lpt_cjh",
            "beta_load": float(beta),
        ...
        rows.append(row)
        pd.DataFrame(rows).to_csv(os.path.join(output_dir, "beta_sweep_actual.csv"), index=False, encoding="utf-8-sig")
        print(row)

    print(f"beta sweep saved: {os.path.join(output_dir, 'beta_sweep_actual.csv')}")
